...
fetch('../webimg/farmer/stand3.svg')
  .then(response => response.text())
  .then(svg => {
    document.getElementById('vendor1').innerHTML = svg;

    const svgElement = document.querySelector('#vendor1 svg');
    svgElement.style.pointerEvents = 'none'; // 整個 svg 不可點

    // 只開放圖形元素可點
    const paths = svgElement.querySelectorAll('path, rect, circle, polygon, g');
    paths.forEach(p => {
      p.style.pointerEvents = 'auto';
      p.style.cursor = 'pointer';
      p.addEventListener('click', (e) => {
        e.stopPropagation(); // 阻止冒泡，避免觸發外層
        alert('你點到攤販1了！');
